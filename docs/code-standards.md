...
- [Formula Rendering System](./formula-rendering.md) — pdflatex single formulas, xelatex combined blocks, MD5 caching, filename conventions
- [Table Rendering Feature](./table-rendering.md) — markdown table detection, LaTeX conversion, PNG rendering
...
### Lesson Delivery (Core Function)

```python
# In src/bot/handlers.py
async def send_lesson_to_chat(bot, chat_id: int, lesson: Lesson):
    """Core delivery function used by both scheduler and command handlers.

    Sends lesson segments, images, and source URL to specified chat.
    This is the canonical delivery function.
    """
    ...
    for segment in segments:
        ...
        else:  # ImagePath (formula, table, or diagram)
            await bot.send_photo(chat_id=chat_id, photo=open(segment, 'rb'))
...
# Wrapper for command handlers (calls send_lesson_to_chat internally)
async def send_lesson(update: Update, context: ContextTypes.DEFAULT_TYPE, lesson: Lesson):
    """Telegram command handler wrapper."""
    await send_lesson_to_chat(context.bot, update.effective_chat.id, lesson)
```

**Note**: Both scheduler and command handlers now use `send_lesson_to_chat()` for consistency. The scheduler imports it directly; command handlers use the `send_lesson()` wrapper.
