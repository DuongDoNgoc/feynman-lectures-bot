...
Command-line interface for lesson preview workflow: export, approve, reject, list.

---

## Context Links

- Depends on: Phase 1 (database), Phase 2 (exporter)
- Related: `scripts/run-enhancer-api.py` (existing CLI pattern)
...
- FR3.1: Export all or specific lessons to markdown
- FR3.2: List lessons with approval status
- FR3.3: Approve/reject lessons from command line
- FR3.4: Sync: re-export only changed lessons
- FR3.5: Show lesson details

### Non-Functional

- NFR3.1: Async execution (use asyncio.run)
- NFR3.2: Colored output for status visibility
- NFR3.3: Exit codes for scripting (0=success, 1=error)
- NFR3.4: Help text for all commands
...
│   ├── --id N      Export specific lesson
│   ├── --type T    Export by type (concept|deep_dive|quiz)
│   └── --all       Export all (default)
├── list            List lessons with status
│   ├── --status S  Filter by approval status
│   └── --type T    Filter by lesson type
├── approve         Mark lesson as approved
│   ├── --id N      Lesson ID (required, unless --all)
│   └── --all       Approve all pending lessons
├── reject          Mark lesson as rejected
│   ├── --id N      Lesson ID (required, unless --all)
│   ├── --all       Reject all pending lessons
│   └── --reason R  Rejection reason
<!-- Updated: Validation Session 1 - Add --all flag to approve/reject -->
├── show            Display lesson content
│   └── --id N      Lesson ID (required)
...
# Reject all pending lessons
python scripts/lesson-preview.py reject --all --reason "batch review"

# Show lesson content
python scripts/lesson-preview.py show --id 5

# Sync changed lessons
...
| File | Purpose |
|------|---------|
| `src/knowledge/preview_db.py` | Database functions |
| `src/content/preview_exporter.py` | Export functions |
...
async def cmd_list(args):
    ...
    for lesson in lessons:
        ...
        print(f"{lesson.id:>4} {lesson.lesson_type:<12} {color_status(status):<19} {title}")
...
def main():
    ...
    approve_parser.add_argument("--id", type=int, required=True, help="Lesson ID")

    # Reject command
    reject_parser = subparsers.add_parser("reject", help="Mark lesson as rejected")
    reject_parser.add_argument("--id", type=int, required=True, help="Lesson ID")
...
```bash
chmod +x scripts/lesson-preview.py
```

### Step 3: Add PyYAML Dependency (if not present)

Check `requirements.txt`:
```
pyyaml==6.0.2  # Should already exist
```
...
- [ ] Create `scripts/lesson-preview.py`
- [ ] Implement `cmd_export()`
- [ ] Implement `cmd_list()`
- [ ] Implement `cmd_approve()` with `--all` flag
- [ ] Implement `cmd_reject()` with `--all` flag
- [ ] Implement `cmd_show()`
- [ ] Implement `cmd_sync()`
...
- [ ] `export` creates markdown files in correct directory
- [ ] `list` shows lessons with colored status
- [ ] `approve --id N` updates database
- [ ] `approve --all` approves all pending lessons
- [ ] `reject --id N` updates database
- [ ] `reject --all` rejects all pending lessons
- [ ] `show --id N` displays lesson content
- [ ] `sync` re-exports only changed lessons
...
- Input validation on lesson IDs
- No shell injection via arguments
- Config loaded from fixed path
...
After completion:
1. Test CLI with existing lessons
